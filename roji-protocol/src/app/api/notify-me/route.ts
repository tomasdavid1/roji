 ...
 *
 * Writes leads to /tmp/notify-me.jsonl (Vercel function tmp is per-
 * invocation but persists for the function lifetime; the canonical
 * source of truth is whatever ESP we wire in later — see
 * roji-tools/FOLLOWUPS.md).
 *
 ...
 * write.
 *
 * Validates email shape and a known toolSlug. Idempotent on
 * (toolSlug, email) within a single function instance.
 */
...
export async function POST(request: Request) {
  ...
  if (!toolSlug || !VALID_SLUGS.has(toolSlug)) {
  ...
  console.log(
    JSON.stringify({ event: "notify_me_capture", ...record }),
  );
